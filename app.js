...
const checkRequestQuery = async (request, response, next) => {
  ...
  if (date !== undefined) {
    try {
      ...
      response.status(400)
      response.send('Invalid Duo Date')
      return
    }
  }
...
app.get('/agenda/', checkRequestQuery, async (request, response) => {
  const {date} = request
  const todo = `SELECT id,todo,priority,status,category,due_date  AS dueDate FROM todo WHERE due_date = ${date};`
  const result = await db.all(todo)
  if (result === undefined) {
    response.status(400)
    response.send('Invalid Due Date')
  } else {
...
app.put('/todos/:todoId/', checkRequestBody, async (request, response) => {
  const {todoId} = request.params
  const {priority, status, todo, category, dueDate} = request.body
